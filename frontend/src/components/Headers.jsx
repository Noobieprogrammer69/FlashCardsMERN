...
const Headers = () => {
  ...
  const logout = async () => {
    try {
      ...
      if(data.error) {
        console.log("Log Out Data Error:", data.error)
      }
    ...
    <header className='flex shadow-md py-4 px-4 sm:px-10 bg-white font-[sans-serif] min-h-[70px] tracking-wide relative z-50'>
      <div className='flex flex-wrap items-center justify-between gap-5 w-full'>
        ...
        <div className='flex max-lg:ml-auto space-x-3'>
          {user ? (
            <>
            <button className='px-4 py-2 text-sm rounded-full font-bold text-white border-2 border-[#007bff] bg-[#007bff] transition-all ease-in-out duration-300 hover:bg-transparent hover:text-[#007bff]'>{user.username}</button>
            <button className='px-4 py-2 text-sm rounded-full font-bold text-white border-2 border-[#007bff] bg-[#ff6f6f] transition-all ease-in-out duration-300 hover:bg-transparent hover:text-[#007bff]' onClick={logout}>Log Out</button>
            </>
          ) : (
            <>
